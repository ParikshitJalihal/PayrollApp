...
namespace Payroll.API.Controllers
{
    ...
    public class TimeSheetEntryController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<TimeSheetEntry>>> GetAll()
        {
            ...
            // populate Task navigation where possible
            foreach (var e in entries)
            {
                if (e.TaskId != 0)
                {
                    e.Task = _unitOfWork.TaskRepo.Get(e => e.TaskId == e.TaskId);
                }
        ...
        public async Task<ActionResult<TimeSheetEntry>> Create([FromBody] TimeSheetEntry dto)
        {
            ...
            if (dto.LogHours < 0m || dto.LogHours > 24m)
                return BadRequest("LogHours must be between 0 and 24.");
            ...
            return CreatedAtAction(nameof(Get), new { id = dto.EntryId }, dto);
        }
